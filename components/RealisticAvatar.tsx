...
// l'écran). 0.80 : assez plat pour la silhouette, assez profond pour que la
// poitrine/le ventre de l'avatar ne transpercent pas le tissu (constaté aussi).
const GARMENT_DEPTH = 0.8
    ...
    <group>
      ...
      {sleeves.map((s) => (
        <mesh
          ...
          castShadow
        >
          <cylinderGeometry args={[0.05, 0.062, s.len, 16, 1, true]} />
          {fabricMat}
        </mesh>
      ))}
      {pantLegs.map((leg, i) => (
